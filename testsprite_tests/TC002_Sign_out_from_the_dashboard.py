...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Masuk' (login) link (element index 42) to open the login page.
        # link "Masuk"
        ...
        # -> Click the user avatar / menu button (index 21102) to open account options so the Sign out option can be clicked.
        # button "B bril"
        elem = page.locator("xpath=/html/body/div[2]/header/nav/div[2]/div/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Keluar' button (index 21101) to sign out and return to the login page.
        # button "Keluar"
        elem = page.locator("xpath=/html/body/div[2]/header/nav/div[2]/div/div/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
