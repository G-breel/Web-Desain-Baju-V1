...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Masuk' (login) link at index 42 to open the login page so credentials can be entered.
        # link "Masuk"
        elem = page.locator("xpath=/html/body/div[2]/header/nav/div/a").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the email and password fields with the supplied credentials and submit the login form by clicking the 'Masuk' button.
        ...
        # -> Open the first design by clicking its 'Buka' link (interactive element index 27837) to navigate to the editor.
        # link "Buka"
        elem = page.locator("xpath=/html/body/div[2]/main/div/div[3]/div/div[3]/a").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Add two canvas objects by clicking the 'Teks' button (index 31607) then 'Duplikat' (index 31609).
        ...
        # -> Select one of the text layers, move it behind the other using the Layer 'Turun' control, then delete the selected object using the 'Hapus' button.
        ...
        # -> Select one of the text layers, move it behind the other using the Layer 'Turun' control, then delete the selected object using the 'Hapus' button.
        # button "Turun"
        elem = page.locator("xpath=/html/body/div[2]/main/div/div[2]/div/aside/div[2]/div[3]/div/button[2]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
