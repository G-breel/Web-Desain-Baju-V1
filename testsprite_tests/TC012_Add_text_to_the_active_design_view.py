...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Masuk' (login) link (element index 42) to open the login page and reveal the login form.
        # link "Masuk"
        elem = page.locator("xpath=/html/body/div[2]/header/nav/div/a").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the email field with admin@gmail.com, fill the password with 123456, then submit the login form by clicking the 'Masuk' button.
        ...
        # -> Click the login/submit button (element index 251) to submit the form and navigate to the dashboard.
        # "Masuk Lanjutkan desain baju kamu Email P..."
        elem = page.locator("xpath=/html/body/div[2]/main/div/div").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Buka' (Open) button for the first design (element index 26415) to open it in the editor.
        # link "Buka"
        elem = page.locator("xpath=/html/body/div[2]/main/div/div[3]/div/div[3]/a").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Teks' (Add Text) button (index 26902) then click the canvas (index 27136) to place the text object and check the UI for the new text object.
